...
import { simplify, distance } from '@turf/turf';
import { sanitizeFileName } from '$lib/export_utils';
import type { RouteEntity } from '$lib/db_data/routes.datatypes';
import type { ExportedRoute } from '$lib/db_data/exported_route.datatypes';
...
function simplifyPointCollection(pointCollection: any, minDistance: number = 0.01): any {
    if (!pointCollection.features || pointCollection.features.length === 0) {
        return pointCollection;
    }

    const simplifiedFeatures = [pointCollection.features[0]]; // Always keep first point
    
    for (let i = 1; i < pointCollection.features.length; i++) {
        const currentPoint = pointCollection.features[i];
        const lastKeptPoint = simplifiedFeatures[simplifiedFeatures.length - 1];
        
        // Calculate distance between points (in kilometers)
        const dist = distance(lastKeptPoint, currentPoint);
        
        if (dist >= minDistance) {
            simplifiedFeatures.push(currentPoint);
        }
    }
    
    // Always keep last point if it's not already included
    const lastPoint = pointCollection.features[pointCollection.features.length - 1];
    if (simplifiedFeatures[simplifiedFeatures.length - 1] !== lastPoint) {
        simplifiedFeatures.push(lastPoint);
    }
...
class ExportProcessorWorkerImpl implements ExportProcessorWorker {
	...
	private async getImageWorker(): Promise<Comlink.Remote<ImageProcessorWorker>> {
		if (!this.imageWorker) {
			this.imageWorker = new Worker(new URL('./imageProcessor.worker.ts', import.meta.url), {
				type: 'module'
			});
	...
	private formatImageNameToURL(filename: string, options: ExportOptions) {
		if (
			options.staticFilesUrls.imagesUrlPrefix === '' &&
			options.staticFilesUrls.imagesUrlSuffix === ''
		) {
			...
			if (options.imageProcessing?.enabled) {
				try {
					...
					onProgress?.(
						...
						totalPhotos,
						mainMessage,
						`Generating optimized variants for ${imageName}...`
					);
					...
					if (processedImage.fallback) {
						...
						onProgress?.(
							...
							totalPhotos,
							mainMessage,
							`Generating JPEG fallback for ${imageName}`
						);
						...
						responsiveImages.fallback = this.formatImageNameToURL(
							`responsive/${fallbackName}`,
							options
						);
					...
					photo.properties.url =
						responsiveImages.fallback ||
						responsiveImages.variants[0]?.src ||
						this.formatImageNameToURL(`original/${imageName}`, options);
				} catch (error) {
					if (
	...
	async processRoutes(
		...
		const totalPhotos = routes.reduce(
			(sum, route) => sum + (route.routeData.photos?.features?.length || 0),
			0
		);
		...
		progressCallback({
			...
			detailedMessage: 'Initializing export process'
		});
		...
		try {
			for (const [routeIndex, route] of routes.entries()) {
				...
				console.log("route data before simplification", route.routeData.route.features[0].geometry.coordinates.length);
				...
				console.log("route data after simplification", simplifiedGeoJSON.features[0].geometry.coordinates.length);
				console.log("sensors data before simplification", route.routeData.sensors.features.length);
				const simplifiedSensorsData = simplifyPointCollection(route.routeData.sensors);
				console.log("sensors data after simplification", simplifiedSensorsData.features.length);
				...
				// build the export entity by removing the visible, originalParsedFitData, originalGPXData, and originalFitData properties
				const exportEntity: ExportedRoute = {
					id: route.id,
					name: route.name,
					date: route.date,
					...
					color: route.color,
					bbox: route.bbox,
					preferedMapStyle: options.defaultMapStyle // Ensure preferedMapStyle is set
				};
