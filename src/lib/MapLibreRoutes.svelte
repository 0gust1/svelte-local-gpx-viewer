<script lang="ts">
	...
	import { type GeoJSONRoute } from '$lib/db';
	import bbox from '@turf/bbox';

	let { geoJSONRoutes }: { geoJSONRoutes: Array<GeoJSONRoute> } = $props();
	let boundingBox = $derived.by(() => {
		let allFeatures = geoJSONRoutes.filter(route=>route.visible).flatMap((route) => route.data.features);
